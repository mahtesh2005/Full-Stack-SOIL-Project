...
async function addReview(username, itemID, description, stars) {
  const mutation = gql`
    mutation($username: String!, $itemID: Int!, $description: String!, $stars: Int!) {
      add_Review(username: $username, itemID: $itemID, description: $description, stars: $stars) {
        username
        itemID
        description
        stars
  ...
  const { data } = await client.mutate({ mutation, variables: {username, itemID, description, stars } });
